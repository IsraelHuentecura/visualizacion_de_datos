...
```{python}
#| label: fig-methods
#| fig-cap: "Histograma aplilado de los diferentes métodos de descubrimientos"
...
```{python}
#| label: fig-count
#| fig-cap: "Barras de cuenta de descubrimientos por método"
#| 
...
plt.xlabel('')
plt.ylabel('')
plt.title('Cantidad de descubrimientos de planetas de los 3 principales métodos')
# Quitar ticks del eje x
plt.xticks([])
# Quitar axis spine bottom
...
#| fig-cap: "Gráfico de dispersión de las variables"
#| 
import seaborn as sns
import matplotlib.pyplot as plt

# el contexto del gráfico para mejorar la estética
...
ax.set_xlabel('Masa del planeta (Masa de la Tierra)')
ax.set_ylabel('Radio del planeta (Radio de la Tierra)')

# Título del gráfico
ax.set_title('Relación entre la Masa y el Radio de los Planetas')

# Crear el scatter plot con incertidumbre en los datos
sns.scatterplot(data=df, x='Masa Planeta', y='Radio Planeta', alpha=0.5, s=100, edgecolor=None, color = 'purple')
...
## Coeficiente de correlación de Pearson y test de hipótesis 
Calculamos cual es la relación lineal de esta variables enfrentadas y probamos la hipótesis de correlación.
...
print(f' y es una relación positiva debido al signo de este')
```

Haciendo el test de hipotesis para saber si hay una correlación este nos dices que se rechaza la hipótesis nula, por lo tanto habria una dependencia lineal entre las variables evaluadas.
Luego el coeficiente nos dice que si hay una dependencia lineal entre estas variables y esa relación es positiva (proporcional) y tiene un valor de 0.307.
